...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    const res = await axios.get(`http://localhost:5500/check-password-link/${resetId}`)
...
async function handleSubmit(event) {
    try {
        ...
        if (newPassword !== confirmPassword)
            ...
            const res = await axios.post(`http://localhost:5500/resetpassword/${resetId}`, { newPassword, confirmPassword })
            // console.log(res)
