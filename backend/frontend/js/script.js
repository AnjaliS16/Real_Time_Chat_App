...
async function renderElemets() {
    try {
        ...
        if (id) {
            ...
            const group = await axios.get(`http://localhost:5500/group/join-group/${id}`, {
                headers: {
        ...
        const res = await axios.get('http://localhost:5500/group/get-groups', {
            headers: {
                'auth-token': localStorage.getItem('token')
            }
        })
...
async function deleteSelectedGroups() {
    ...
    try {
        ...
        await Promise.all(Array.from(checkboxes).map(async checkbox => {
            ...
            const res = await axios.delete(`http://localhost:5500/group/delete/${groupId}`, {
                headers: {
                    'auth-token': localStorage.getItem('token')
                }
            });
...
async function handle(event) {
    ...
    try {
        ...
        const res = await axios.post('http://localhost:5500/message/add-message', data, {
            headers: {
                'auth-token': localStorage.getItem('token')
            }
        });
...
async function createNewGroup(e) {
    try {
        ...
        const group = await axios.post('http://localhost:5500/group/create', { "name": e.target.name.value, selectedUsers }, {
            headers: {
                'auth-token': localStorage.getItem('token')
            }
        })
...
document.getElementById('create-grp').addEventListener('click', async () => {
    if (document.querySelector('.new-group').classList.contains('hide')) {
        ...
        const res = await axios.get('http://localhost:5500/group/other-users', {
            headers: {
                'auth-token': localStorage.getItem('token')
            }
        })
...
async function showGroupMessages() {
    ...
    try {
        ...
        const res = await axios.get(`http://localhost:5500/message/get-messages/${group.id}/?messageId=${mId}`, {
            headers: {
                'auth-token': localStorage.getItem('token')
            }
        })
        const res2 = await axios.get(`http://localhost:5500/group/all-users/${group.id}/?id=${uId}`, {
            headers: {
                'auth-token': localStorage.getItem('token')
            }
        })
...
console.log(filteredUsers,'>>>>>>>>>>>>filteredUsers')
        ...
        const res3 = await axios.post(`http://localhost:5500/admin/show-users/${group.id}`, null, {
            headers: {
                'auth-token': localStorage.getItem('token')
            }
        })
...
function showUser(user) {
    ...
    if (user.member.id != member.id && member.admin) {
        ...
        makeAdmin.onclick = async () => {
            try {
                const res = await axios.post(`http://localhost:5500/admin/make-admin/${curr_group.id}`, { "userId": user.id }, {
                    headers: {
                        'auth-token': localStorage.getItem('token')
                    }
                })
        ...
        removeAdmin.onclick = async () => {
            try {
                const res = await axios.post(`http://localhost:5500/admin/remove-admin/${curr_group.id}`, { "userId": user.id }, {
                    headers: {
                        'auth-token': localStorage.getItem('token')
                    }
                })
        ...
        removeUser.onclick = async () => {
            try {
                const res = await axios.post(`http://localhost:5500/admin/remove-member/${curr_group.id}`, { "userId": user.id }, {
                    headers: {
                        'auth-token': localStorage.getItem('token')
                    }
                })
...
function addUser(user) {
    ...
    btn.onclick = async () => {
        try {
            ...
            const res = await axios.post(`http://localhost:5500/admin/add-user/${curr_group.id}`, {
                id: user.id
            }, {
                headers: {
                    'auth-token': localStorage.getItem('token')
            ...
            console.log(res.data.user[0], 'from adduser')
           
            showUser(show_user)
            
          

             var removedDataArray =  retrieveRemovedDataArray();
             removedDataArray = [];
             localStorage.setItem('removedDataArray', JSON.stringify(removedDataArray));
...
document.getElementById('files').addEventListener('submit', async (e) => {
    try {
        ...
        const res = await axios.post(`http://localhost:5500/message/upload-file/${group.id}`, formData, {
            headers: {
                'auth-token': localStorage.getItem('token')
            }
        })
